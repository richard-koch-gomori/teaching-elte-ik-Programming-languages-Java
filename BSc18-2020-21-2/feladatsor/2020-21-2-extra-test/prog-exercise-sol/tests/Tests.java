...
public class Tests
{
    ...
    {
        Cipher.encrypt("password", -1); // 0.5 pont
    }

    @Test
    public void Encrypt_test_pos_shift()
    {
        assertEquals("qbttxpse", Cipher.encrypt("password", 1)); // 0.5 pont
    }

    @Test
    public void Decrypt_test_positive_shift()
    {
        assertEquals("password", Cipher.decrypt("qbttxpse", 1)); // 0.5 pont
    }
    ...
    @Test(expected = ServerException.class) // 0.5 pont
